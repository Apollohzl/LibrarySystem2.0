...
class Library:
    def __init__(self):
        ...
        self.LIBRARY_TIME_OUT = 15#10*60 #10分钟
        pass
#日志添加
    ...
    def list_Logs(self)->list:
        print(f">Library>list_Logs()")
        tosql = '''
        SELECT * from log
        '''
        cursor = systemlog.execute(tosql)
...
#借书
    def Borrow_Book(self,isbn:str,usermsg:list,save_history=False):
        print(f">Library>Borrow_Book({isbn} {usermsg} {save_history})")
                ...
                if usermsg != []:
                    if self.Login_User_Has_Book(usermsg,isbn) == False:
                        ...
                        if save_history == True:
                            ...
                            for i in str(time.time()).split('.'):
                                uid += i
                            for i in range(random.randint(0,len(uid)-1)):
                                l = random.randint(0,len(uid)-1)
                                uid = uid[:l] + random.choice(string.ascii_letters) + uid[l+1:]
                            tosql = '''
                            ...
                            msgtosql = (
                                ...
                                isbn,
                                datetime.datetime.now(),
                                datetime.datetime.now()+datetime.timedelta(seconds=self.LIBRARY_TIME_OUT),
                                usermsg[0],
                                usermsg[1],
                                usermsg[2],
                                uid
...
#用户信息修改
    def Modify_User_Info(self,userauthermsg:list,useruid,makenewqrcode=False,shownewqrcode = False,save_path=f"{mypath('学生信息\\')}"):
        print(f">Library>Modify_User_Info({userauthermsg},{useruid},{makenewqrcode},{shownewqrcode},{save_path})")
        tosql = '''
        UPDATE users set Username =?,Userid =?,Userclass =?,UserBorrowBooks =?,UserPassword =?,UserBorrowedBooks =? where Useruid =?'''
        msgtosql = (
            userauthermsg[0],
            userauthermsg[1],
            userauthermsg[2],
        ...
        Librarysql.execute(tosql,msgtosql)
        Librarysql.commit()
        self.Add_Log(f"User {userauthermsg} 修改信息")
        if makenewqrcode == True:
            encrypt.自动化加密并二维码(userauthermsg[0],userauthermsg[2],userauthermsg[1],userauthermsg[4],shownewqrcode,save_path)
        print(f">Library>Modify_User_Info()-R:Modify Ok!")
        return True
#list正在借书
    def List_Borrowing(self)->list:
        print(">Library>List_Borrowing()")
        tosql = '''SELECT bookname,ReaderName,Readerclass,Readerid,borrowtime, MustReturnTime, borrowid FROM borrow'''
        cursor = Librarysql.execute(tosql)
        tr = cursor.fetchall()
        return tr
    def List_Borrow_History(self)->list:
        print(">Library>List_Borrow_History()")
        tosql = '''SELECT bookname,ReaderName,Readerclass,Readerid,borrowtime,MustBookReturnTime FROM borrowhistory'''
        cursor = Librarysql.execute(tosql)
        tr = cursor.fetchall()
        print(tr)
        # tr[i].append("已归还")
        return tr
    def List_ReturnTimeOut(self)->list:
        print(">Library>List_ReturnTimeOut()")
        tosql = '''SELECT bookname,ReaderName,Readerclass,Readerid,borrowtime,MustReturnTime,borrowid FROM borrow'''
        cursor = Librarysql.execute(tosql)
        tr = cursor.fetchall()
        r = []
        for i in tr:
            time_ob = datetime.datetime.strptime(i[5], '%Y-%m-%d %H:%M:%S.%f')
            time_ob2 = datetime.datetime.now()
            if time_ob < time_ob2:
                r.append(i)
        return r
    def delete_borrowing_or_borrowouttime_msg(self,borrowid)->bool:
        print(f">Library>delete_borrowing_or_borrowouttime_msg({borrowid})")
        tosql = '''DELETE from borrow where borrowid =?'''
        msgtosql = (borrowid,)
        Librarysql.execute(tosql,msgtosql)
        Librarysql.commit()
        self.Add_Log(f"去除借阅信息 {borrowid} 书籍")
        return True
